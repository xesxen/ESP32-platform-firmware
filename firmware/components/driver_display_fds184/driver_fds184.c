...
#include "include/driver_fds184.h"
#include "include/compositor.h"

#include "esp_log.h"

#ifdef CONFIG_DRIVER_FDS184_ENABLE
		...
		for (unsigned int y=0; y<8; y++) {
			...
			for (int fx=0; fx<32; fx++) {
				...
				if (hub75_framebuffer) {
					c1 = hub75_framebuffer[yreal*CONFIG_FDS184_WIDTH+xreal];
				} else {
...
{
	static bool driver_fds184_init_done = false;
	if (driver_fds184_init_done) return ESP_OK;
...
#else // DRIVER_FDS184_ENABLE
esp_err_t driver_fds184_init(void) { return ESP_OK; } // Dummy function, leave empty!
#endif
