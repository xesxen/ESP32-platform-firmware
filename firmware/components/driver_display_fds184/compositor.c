...
void compositor_setPixel(int x, int y, Color color) {
	if (!buffer) return;
	Color *target = &buffer[y*CONFIG_FDS184_WIDTH+x];
...
void renderImage(uint8_t *image, int x, int y, int sizeX, int sizeY) {
	...
	for(int py=0; py<sizeY; py++) {
		...
		for(int px=0; px<sizeX; px++) {
			xreal = x + px;
			if(yreal >= 0 && yreal < CONFIG_FDS184_HEIGHT && xreal >= 0 && xreal < CONFIG_FDS184_WIDTH) {
...
void renderText(char *text, Color color, int x, int y, int sizeX, int skip, bool firstshow) {
	int endX = sizeX > 0 ? x+sizeX : CONFIG_FDS184_WIDTH - 1;
	if(skip < 0) {
		if (!firstshow)
		{
			x += -skip;
...
void display_crash() {
	...
	for(int x=0; x<CONFIG_FDS184_WIDTH; x++) {
		for(int y=0; y<CONFIG_FDS184_HEIGHT; y++) {
			buffer[y*CONFIG_FDS184_WIDTH+x] = blue;
		}
	}
...
void composite() {
	...
	for(int x=0; x<CONFIG_FDS184_WIDTH; x++) {
			for(int y=0; y<CONFIG_FDS184_HEIGHT; y++) {
					buffer[y*CONFIG_FDS184_WIDTH+x] = background;
			}
	}
	renderTask_t *node = head;
	while(node != NULL) {
